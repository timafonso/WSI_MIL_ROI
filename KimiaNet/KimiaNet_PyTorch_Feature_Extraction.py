...
def test_model(model, criterion, num_epochs=25):
	...
	for i in range(1):
		...
		test_path_test = './tiles/TCGA-77-7335-01A-01-BS1.12e63031-647d-4755-888e-4a4f25f5c36c'
		test_imagedataset = My_dataloader(test_path_test,trans)
		dataloader_test = torch.utils.data.DataLoader(test_imagedataset, batch_size=16,shuffle=False, num_workers=0)
		counter = 0
		# Iterate over data.
...
model_final.load_state_dict(torch.load('KimiaNet/KimiaNet_Weights/weights/KimiaNetPyTorchWeights.pth', map_location=torch.device('cpu')))
